...
@section('content')
    <main class="mx-auto max-w-3xl px-5 py-16 text-center lg:px-8">
        <section class="rounded-3xl bg-[#fff0bd] p-8 sm:p-12">
            ...
            <div class="mx-auto mt-7 max-w-md rounded-2xl bg-white p-5 text-left"><p class="text-sm text-black/60">Payment Method</p><p class="mt-1 font-bold">{{ $order->payment_method === 'razorpay' ? 'Online Payment (Razorpay)' : 'Cash on Delivery' }}</p><p class="mt-4 text-sm text-black/60">Payment Status</p><p class="mt-1 font-bold text-emerald-700">{{ ucfirst($order->payment_status) }}</p><p class="mt-4 text-sm text-black/60">Order Total</p><p class="mt-1 text-2xl font-black">₹{{ number_format((float) $order->total, 2) }}</p></div>
